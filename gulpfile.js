...
gulp.task("pugToHtmlEnglish", () => {
  return gulp
    .src("./src/pug/*.pug")
    .pipe(pug({ pretty: true }).on("error", notify.onError("Error: <%= error.message %>")))
    .pipe(gulp.dest("./dist/"))
    .pipe(browserSync.stream());
});
...
gulp.task("default", gulp.series("sass", "pugToHtmlEnglish", "imgSquash", "fonts", "javaScript", "cssVendor", "serve"));
